...
class videoFrameRead:

    def calc_percentage(self, values, length):
        frequency = dict(pd.value_counts(values))
        for count in frequency:
            calc = round((((frequency[count]/length))*100), 2)
    ...
    def facialEmtions(self, link):
        ...
        ap.set_defaults(savedata=True)
        ap.set_defaults(save=False)
        args = vars(ap.parse_args())

        videoFilePath = link
        dirName=os.getcwd()
        #path = "C:/Users/Mudassir/Desktop/Emotion_recognition/"
        vidcap = FileVideoStream(videoFilePath).start()

        framecount = 0
        learn = load_learner(dirName,'export.pkl')
        data = []
        expressionsList = []
        ...
        if args["save"]:
            out = cv2.VideoWriter(
                dirName + "output.avi", cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (450, 300))
            ...
            for coords in face_coord:
                ...
                try:
                    if framecount % args["frame_step"] == 0:
                        ...
                        data.append([framecount, prediction, probability])
                        expressionsList.append(str(prediction))
                    else:
                        prediction, idx, probability = ""
                    cv2.rectangle(
                        img=frame,
                        pt1=(X_1, Y_1),
                        pt2=(X_2, Y_2),
        ...
        if args["savedata"]:
            ...
            expressionsFreq = self.calc_percentage(
                expressionsList, len(expressionsList))
            # pprint(expressionsFreq)
        vidcap.stop()
        # if args["save"]:
        #     print("done saving")
        #     out.release()
        cv2.destroyAllWindows()
        return expressionsFreq
        # with open("export.csv") as csv_file:
        #     csv_reader=csv.DictReader(csv_file, delimiter=',')
        #     for lines in csv_reader:
        #         expressionsList.append(lines['Expression'])
